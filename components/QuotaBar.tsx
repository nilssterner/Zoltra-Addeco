'use client'
import { UserQuota, leadsLeft, mailLeft, leadsLimit, mailLimit } from '@/lib/quota'
    ...
    <div className={`rounded-xl border p-4 space-y-3 ${anyFull ? 'border-red-200 bg-red-50' : anyWarn ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'}`}>
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
          Kvot – {plan.name}
          {plan.isOneTime && <span className="ml-1.5 text-slate-400 normal-case font-normal">(engångskvot)</span>}
        </span>
      ...
      {anyFull && (
        <p className="text-xs text-red-700 bg-red-100 rounded-lg px-3 py-2">
          ...
          <a href="#pris" className="font-semibold underline">Uppgradera din plan</a>{' '}
          för att fortsätta.
        </p>
      )}
      {!anyFull && anyWarn && (
        <p className="text-xs text-amber-700 bg-amber-100 rounded-lg px-3 py-2">
          ...
          <a href="#pris" className="font-semibold underline">Uppgradera</a>{' '}
          i god tid.
        </p>
      )}
    </div>
