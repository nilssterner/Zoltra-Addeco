...
export default function SequenceBuilder() {
  ...
  const save = async () => {
    ...
    const invalid = steps.some(s => !s.subject.trim() || !s.body.trim())
    if (invalid) { setError('Fyll i ämne och innehåll för varje steg'); return }
    setError('')
    setSaving(true)
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <input className={inputClass} value={name} onChange={e => setName(e.target.value)} placeholder="t.ex. Bygg-outreach v1" />
      </div>
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-slate-700">
                  {i === 0 ? 'Första mailet' : `Uppföljning ${i}`}
                </span>
              </div>
              {i > 0 && (
                <button onClick={() => removeStep(i)} className="text-xs text-slate-400 hover:text-red-500 transition-colors">Ta bort</button>
              )}
            </div>

            {i > 0 && (
              <div className="mb-3">
                <label className="block text-xs font-medium text-slate-600 mb-1">Skickas efter (dagar från föregående)</label>
                <input
      ...
      {saved && <p className="text-sm text-green-600">Sekvens sparad!</p>}
